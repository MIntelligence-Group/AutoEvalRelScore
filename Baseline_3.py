...
df = pd.read_csv("train.csv", index_col="id", nrows = 100)
...
class SimilarityModule(nn.Module):
    def __init__(self):
        ...
        self.gru_1 = nn.GRU(100, 768, dropout=0.2, batch_first=True)
        self.gru_2 = nn.GRU(100, 768, dropout=0.2, batch_first=True)
        self.bert_drop = nn.Dropout(0.2)
        self.f_out = nn.Linear(768, 1)
    ...
    def forward(self, ids, mask, token_type_ids, q1, q2):
        ...
        _, q1_context = self.gru_1(q1)
        _, q2_context = self.gru_2(q2)
        q1 = (q1_context+q2_context)/2
        
        ensembled = (q1+o1)/2
        out = self.sigmoid(self.f_out(ensembled)).to(device)
        return out
