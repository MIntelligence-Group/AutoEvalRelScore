...
---------------------
...
1. We have three sets of csv files comprising of training data, validation data and test data.
2. The test feedbacks are generated by running the pre-trained model described in the [Base Paper](add link). We get the csv file ```test_feedbacks.csv``` as a result.These feedbacks are then fed into the Similarity Module to generate the similarity scores.
